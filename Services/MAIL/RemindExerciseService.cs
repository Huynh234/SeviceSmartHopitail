...
{
    public class RemindExerciseService : BackgroundService
    {
        private readonly IServiceScopeFactory _scopeFactory;

        public RemindExerciseService(IServiceScopeFactory scopeFactory)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    using (var scope = _scopeFactory.CreateScope())
                    {
                        ...
                        var reminders = db.RemindExercises
                        ...
                        foreach (var reminder in reminders)
                        {
                            ...
                            string subject = "Nhắc nhở tập thể dục định kỳ";
                            string body = reminder.Title ?? "Hãy tập thể dục đúng giờ để có cơ thể khỏe mạnh.";
                            ...
                            try
                            {
                                ...
                                db.RemindExercises.Update(reminder);
                            }
